...
//我觉的这里并不需要在free后合并，因为每一次free之后，都将这一块加入到链表的开头
//我觉得这里的合并需要考虑在分配新块之前，即extend_heap之前，看链表中是否有需要合并的
//如果合并之后的链表节点符合需要分配的大小，那么就暂时不需要分配新的块
//但是这里找地址都是连续的两个链表节点，针对我实现的每一次插入都是放在头节点是比较麻烦的
//需要使用hash把链表节点全都存在里面，方便查找是否有前后可以合并的
//也可以实现插入是按照地址顺序来插入
static void *coalesce(void *bp)
{
